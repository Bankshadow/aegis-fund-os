...
import { computeRealizedCycles } from "@/lib/grid-realized";
...
export const Route = createFileRoute("/bots_/$botId_/profit")({
  ...
  loader: async ({ params }) => {
    ...
    return { bot, projections: bot ? orders.map((order) => projectGridCycleProfit(bot, order)) : [], realized: bot ? computeRealizedCycles(bot, orders) : null, exchangeStatus, exchangeError };
  },
  component: GridProfitPage,
});

function GridProfitPage() {
  const { bot, projections, realized, exchangeStatus, exchangeError } = Route.useLoaderData();
  ...
  return <AppShell><PageHeader kicker={botId} title="Grid Profit per Cycle" subtitle={bot ? `${bot.name} ยท fee-aware Testnet projection` : "Bot not found"} actions={<div className="flex gap-2">{canReconcile && <Button disabled={reconciling} onClick={reconcile}>{reconciling ? "Reconciling..." : "Reconcile fills"}</Button>}<Button variant="outline" asChild><Link to="/bots/$botId" params={{ botId }}>Bot detail</Link></Button></div>} />
    <div className="space-y-6 p-6"><Panel title="Profit status" subtitle="Only completed and reconciled fills can become realized P/L."><div className="grid gap-3 md:grid-cols-3">
      ...
      <div className="rounded-md border p-4"><div className="text-xs text-muted-foreground">Realized P/L (completed cycles)</div><div className="mt-1 text-2xl font-semibold text-positive">{realized ? `${realized.realizedProfit} USDT` : "—"}</div><div className="mt-1 text-xs text-muted-foreground">{realized ? `${realized.matchedCycles} matched · ${realized.openBuyLegs + realized.openSellLegs} open legs · fees ${realized.allFeesActual ? "from real commissions" : "partly estimated"}` : ""}</div></div>
    </div>{realized && realized.cycles.length > 0 && <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[560px] text-sm"><thead><tr className="text-left text-xs uppercase text-muted-foreground">{["Buy fill", "Sell fill", "Quantity", "Fees", "Fee basis", "Realized profit"].map((heading) => <th className="p-2" key={heading}>{heading}</th>)}</tr></thead><tbody>{realized.cycles.map((cycle, index) => <tr className="border-t" key={index}><td className="p-2 font-mono">{cycle.buyPrice}</td><td className="p-2 font-mono">{cycle.sellPrice}</td><td className="p-2 font-mono">{cycle.quantity}</td><td className="p-2 font-mono">{cycle.fees}</td><td className="p-2 text-xs text-muted-foreground">{cycle.feeBasis === "actual" ? "real commission" : "estimated 0.10%"}</td><td className={`p-2 font-mono ${Number(cycle.profit) < 0 ? "text-destructive" : "text-positive"}`}>{cycle.profit} USDT</td></tr>)}</tbody></table></div>}<p className="mt-4 text-sm text-muted-foreground">{exchangeStatus ? `Exchange verified ${exchangeStatus.matchingOpenOrderCount}/${exchangeStatus.ledgerOrderCount} open orders · ${exchangeStatus.matchingTradeCount} fills observed · ${new Date(exchangeStatus.checkedAt).toLocaleString()}` : exchangeError || "Current orders are exchange acknowledgements, not fills. No projected value is presented as realized P/L."}</p></Panel>
